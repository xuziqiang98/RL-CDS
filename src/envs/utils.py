...
# 所有的状态种类
DEFAULT_OBSERVABLES = [Observable.VERTEX_STATE, # 顶点的状态
                       Observable.IMMEDIATE_REWARD_AVAILABLE, # 改变顶点后的直接奖励
                       Observable.TIME_SINCE_FLIP, # 上一次变化后的行动
                       Observable.DISTANCE_FROM_BEST_SCORE, # 与最优解的值之间的差距
                       Observable.DISTANCE_FROM_BEST_STATE, # 当前解集与最优解之间的距离
                       Observable.NUMBER_OF_GREEDY_ACTIONS_AVAILABLE, # 可以立刻增加解的值的可采取的行动
                       Observable.TERMINATION_IMMANENCY] # 剩余的回合数

class GraphGenerator(ABC):

    def __init__(self, n_vertices, edge_type, biased=False):
...
class RandomGraphGenerator(GraphGenerator):
    ...
    def __init__(self, n_vertices=20, edge_type=EdgeType.DISCRETE, biased=False):
        super().__init__(n_vertices, edge_type, biased)
    ...
    def get(self, with_padding=False):
        ...
        if self.biased:
            bias = np.array([self.get_w() if np.random.uniform() < density else 0 for _ in range(self.n_vertices)])
...
class RandomErdosRenyiGraphGenerator(GraphGenerator):
    ...
    def __init__(self, n_vertices=20, p_connection=[0.1,0], edge_type=EdgeType.DISCRETE):
        ...
        if self.edge_type == EdgeType.UNIFORM:
            self.get_connection_mask = lambda : np.ones((self.n_vertices,self.n_vertices))
        elif self.edge_type == EdgeType.DISCRETE:
            def get_connection_mask():
                mask = 2. * np.random.randint(2, size=(self.n_vertices, self.n_vertices)) - 1.
                mask = np.tril(mask) + np.triu(mask.T, 1)
                return mask
            self.get_connection_mask = get_connection_mask
        elif self.edge_type == EdgeType.RANDOM:
            def get_connection_mask():
                mask = 2.*np.random.rand(self.n_vertices,self.n_vertices)-1
                mask = np.tril(mask) + np.triu(mask.T, 1)
                return mask
            self.get_connection_mask = get_connection_mask
        else:
...
class RandomRegularGraphGenerator(GraphGenerator):
    ...
    def __init__(self, n_vertices=20, d_node=[2,0], edge_type=EdgeType.DISCRETE, biased=False):
        super().__init__(n_vertices, edge_type, biased)
...
class RandomWattsStrogatzGraphGenerator(GraphGenerator):
    ...
    def __init__(self, n_vertices=20, k_neighbours=[2,0], edge_type=EdgeType.DISCRETE, biased=False):
        super().__init__(n_vertices, edge_type, biased)
...
class SingleGraphGenerator(GraphGenerator):
    ...
    def __init__(self, matrix, bias=None):
        ...
        super().__init__(n_vertices, edge_type, bias is not None)
...
class SetGraphGenerator(GraphGenerator):
    ...
    def __init__(self, matrices, biases=None, ordered=False):
        ...
        if all([np.isin(m,[0,1]).all() for m in matrices]):
            edge_type=EdgeType.UNIFORM # 矩阵的元素取0或1
        elif all([np.isin(m,[0,-1,1]).all() for m in matrices]):
            edge_type=EdgeType.DISCRETE # 矩阵的元素取-1，0或1
        else:
            edge_type = EdgeType.RANDOM

        super().__init__(n_vertices, edge_type, biases is not None)

        if not self.biased:
            self.graphs = matrices
        else:
            assert len(matrices)==len(biases), "Must pass through the same number of matrices and biases."
            assert all([len(b)==self.n_vertices+1 for b in biases]), "All biases and must have the same dimension as the matrices."
...
class PerturbedGraphGenerator(GraphGenerator):
    ...
    def __init__(self, matrices, perturb_mean=0, perturb_std=0.01, biases=None, ordered=False):
        ...
        super().__init__(n_vertices, EdgeType.RANDOM, biases is not None)
...
class HistoryBuffer():
    ...
    def update(self, action):
        '''
        current_action_hist里保存了所有执行过的动作
        传进来的action就是本次选择的顶点
        如果该顶点已经被选过了，再次选择就相当于又移出了解集，所以要去掉
        否则就加入解集
        new_action_hist是本次操作完action后的解集，current_action_hist是之前的解集
        list_of_states应该是所有长度相同的解集的一个list
        只有当新的解集是第一次出现才会更新并返回True
        否则不更新buffer返回False
        '''
        new_action_hist = self.current_action_hist.copy() # new_action_hist是集合
        ...
        try:
            ...
            if new_action_hist in list_of_states: # list_of_states是list，每一个元素都是一个集合
                self.current_action_hist = new_action_hist
                return False # 这里是选择action后的解集以前出现过，那就返回False
        except KeyError:
            list_of_states = []
        ...
        return True # 如果解集是第一次出现，就更新buffer，返回True